...
window.FloodCauseEvolutionModule = class FloodCauseEvolutionModule {
    ...
    for (const basin of this.basins) {
      ...
      for (const shift of shifts) {
        ...
        if (hovered) {
          ...
          ctx.lineWidth = 4.2;
          ctx.stroke();
          ctx.shadowColor = "rgba(217,70,239,.72)";
          ctx.shadowBlur = 12;
        ...
        if (hovered) {
          ...
          const tier = metric.largerSample ? "≥20 catchments" : "5–19 catchments";
          if (center.x > -100 && center.x < viewport.width + 100) this.drawHoverLabel(ctx, center.x, center.y, `${basin.code} · ${this.signed(metric.slope)} pp/dec · ${tier}`);
        }
      }
    }
  }
    ...
    for (const catchment of this.catchments) {
      ...
      for (const shift of shifts) {
        ...
        if (hovered) {
          ...
          ctx.lineWidth = 4;
          ctx.stroke();
          ctx.shadowColor = "rgba(217,70,239,.78)";
          ctx.shadowBlur = 10;
          ctx.strokeStyle = "#D946EF";
          ctx.lineWidth = 2.25;
          ctx.stroke();
  ...
  drawHoverLabel(ctx, x, y, text) {
    ...
    ctx.fillText(text, left + 10, top + height / 2 + 0.5);
    ctx.restore();
  }
  ...
  updateLegend() {
    ...
    this.app.registerLegend?.(this.legendId, {
      ...
      html: `
        ...
        <div class="fce-legend-key"><i class="boundary"></i> Included HydroBASINS boundary</div>
        <div class="fce-legend-key"><i class="hover"></i> Hover highlight</div>
  ...
  showInspector(feature, layerId) {
    ...
    if (layerId === this.basinLayerId) {
      const limitedSample = metric && !metric.largerSample;
      const tierLabel = metric?.highConfidence
        ? `High-confidence regional signal${limitedSample ? " · limited sample" : ""}`
        : limitedSample
        ? "Primary estimate · limited sample (5–19 catchments)"
        : "Primary estimate · larger sample (≥20 catchments)";
      this.app.showInspector?.(`${feature.code} · ${feature.countries}`, `
        ...
        <div class="fce-status ${metric?.highConfidence ? "pass" : limitedSample ? "limited" : "neutral"}">${tierLabel}</div>
        ${this.signal(metric)}
        <div class="fce-grid">
          ${this.fact("95% CI", metric ? `${this.num(metric.ci[0])} to ${this.num(metric.ci[1])}` : "—")}
          ${this.fact("Primary-family q (≥5)", this.prob(metric?.analysisQ))}
        ...
        ${this.gates(metric)}
        <p class="fce-note">${limitedSample
          ? `This unit meets the formal ≥${this.data.meta.analysisMinimumCatchments}-catchment and ≥${this.data.meta.minimumObservations}-observation analysis floor, but its cluster-robust uncertainty is based on fewer than ${this.data.meta.largerSampleCatchments} catchments; interpret it more cautiously.`
          : "This unit has at least 20 catchments, providing stronger cluster support."} Solid outlines identify signals that passed the prespecified high-confidence screen; nested-scale stability remains a separate diagnostic. Color alone is not a significance claim.</p>`);
      return;
    }
  ...
  gates(metric) {
    if (!metric) return "";
    const gates = [
      ["Primary FDR", Number(metric.analysisQ) < 0.05],
  ...
  showOverview() {
    ...
    this.modal.querySelector(".fce-modal-body").innerHTML = `
      ...
      <div class="fce-kpis">
        ${this.kpi(this.data.meta.eligibleHydrobasins, "mapped L5 regions (≥5)")}
        ${this.kpi(this.data.meta.limitedSampleHydrobasins, "limited-sample L5 regions")}
        ${this.kpi(this.data.meta.largerSampleHydrobasins, "larger-sample L5 regions")}
      ...
      <section><h3>How to read the map</h3><div class="fce-reading">
        ...
        <div><b>Evidence boundary</b><span>All included regions use quiet black boundaries; thicker outlines mark high-confidence signals. Sample tier is stated on hover and in the inspector.</span></div>
      </div></section>
  ...
  ensureStyles() {
    ...
    style.id = "fce-styles";
    style.textContent = `
      .panel-section-header{font-size:12px}.module-item,.layer-item,.layer-expandable,.layer-sublayer,.map-display-option{font-size:13px}.legend-card{font-size:12px;padding:12px 13px}.legend-title{font-size:13px}.inspector-title{font-size:15px}.inspector-body{font-size:13px;line-height:1.5}
      .fce-toolbar{position:fixed;top:14px;left:50%;transform:translateX(-50%);z-index:850;display:flex;align-items:center;gap:10px;padding:9px 10px 9px 14px;border:1px solid rgba(148,163,184,.35);border-radius:14px;background:rgba(255,255,255,.94);box-shadow:0 10px 30px rgba(15,23,42,.12);backdrop-filter:blur(16px);font:13px/1.25 Inter,system-ui,sans-serif;color:#334155}.fce-toolbar-label{font-weight:700;white-space:nowrap}.fce-segmented{display:flex;padding:3px;border-radius:10px;background:#edf1f4}.fce-segmented button,.fce-overview-button{border:0;border-radius:8px;padding:8px 11px;background:transparent;color:#526171;font:600 12px Inter,system-ui;cursor:pointer}.fce-segmented button.active{background:#fff;color:#173d55;box-shadow:0 2px 8px rgba(15,23,42,.1)}.fce-overview-button{background:#173d55;color:#fff}.fce-legend-scale-title{margin:6px 0 5px;font-size:12px;font-weight:700;color:#475569}.fce-legend-scale-title.catchment{margin-top:11px}.fce-legend-bar{height:11px;border-radius:99px;background:linear-gradient(90deg,#275f83,#ece7d8 50%,#c75c36)}.fce-legend-axis{display:flex;justify-content:space-between;margin-top:4px;font:11px/1.25 ui-monospace,monospace;color:#64748b}.fce-legend-unit{margin:8px 0 9px;font-size:11px;line-height:1.45;color:#64748b}.fce-legend-key{display:flex;align-items:center;gap:8px;margin-top:7px;font-size:11px;line-height:1.35;color:#475569}.fce-legend-key i{display:inline-block;flex:0 0 auto;width:19px;height:10px}.fce-legend-key .solid{border:2px solid #172235;background:#d5c9a8}.fce-legend-key .boundary{border:1px solid #172235;background:#d5c9a8}.fce-legend-key .hover{border:2px solid #D946EF;background:#d5c9a8;box-shadow:0 0 7px rgba(217,70,239,.65)}.fce-legend-key .dot{width:9px;height:9px;border-radius:50%;background:#527b95;border:1px solid #334155}.fce-inspector-lead,.fce-note{font-size:12px;line-height:1.6;color:#64748b}.fce-signal{margin:13px 0;padding:15px;border-radius:12px;background:#f4f6f7}.fce-signal span,.fce-signal small{display:block;color:#64748b;font-size:11px}.fce-signal strong{display:block;margin:6px 0 3px;font-size:29px;letter-spacing:-.04em}.fce-grid{display:grid;grid-template-columns:1fr 1fr;gap:9px}.fce-fact{padding:11px;border:1px solid #e2e8f0;border-radius:10px}.fce-fact span,.fce-fact strong{display:block}.fce-fact span{font-size:11px;line-height:1.3;text-transform:uppercase;letter-spacing:.035em;color:#6f7f8e}.fce-fact strong{margin-top:5px;font-size:13px;color:#263646}.fce-gates{display:flex;flex-wrap:wrap;gap:7px;margin:13px 0}.fce-gates span,.fce-status{padding:6px 9px;border-radius:99px;font:600 11px/1.25 Inter,system-ui}.fce-gates .pass,.fce-status.pass{background:#e4eee7;color:#315e45}.fce-gates .fail,.fce-status.neutral{background:#eef1f3;color:#788592}.fce-status.limited{background:#f4ede0;color:#80612e}.fce-status{display:inline-block;margin-top:13px}.fce-modal{position:fixed;inset:0;z-index:1100;display:none;align-items:center;justify-content:center;padding:28px;background:rgba(15,23,42,.48);backdrop-filter:blur(9px)}.fce-modal.visible{display:flex}.fce-modal-card{position:relative;width:min(900px,94vw);max-height:90vh;overflow:auto;border:1px solid rgba(255,255,255,.5);border-radius:22px;background:#f8fafb;box-shadow:0 32px 90px rgba(15,23,42,.28);color:#223143;font:14px/1.65 Inter,system-ui,sans-serif}.fce-close{position:sticky;float:right;top:14px;margin:14px 14px -50px 0;z-index:2;width:34px;height:34px;border:0;border-radius:50%;background:#e7ecef;color:#415161;font-size:22px;cursor:pointer}.fce-modal-body{padding:40px}.fce-hero{padding:4px 48px 26px 0;border-bottom:1px solid #dce3e8}.fce-kicker{font-size:11px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:#9b6a26}.fce-hero h2{max-width:680px;margin:9px 0 12px;font-size:34px;line-height:1.08;letter-spacing:-.04em;color:#172638}.fce-hero p{max-width:720px;margin:0;color:#5f6f7e}.fce-kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:20px 0}.fce-kpis div{padding:15px;border:1px solid #dce3e8;border-radius:13px;background:#fff}.fce-kpis strong,.fce-kpis span{display:block}.fce-kpis strong{font-size:22px;color:#173d55}.fce-kpis span{margin-top:4px;font-size:11px;color:#73818f}.fce-modal section h3{margin:26px 0 8px;font-size:16px;color:#243648}.fce-reading{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.fce-reading div{padding:15px;border-radius:13px;background:#edf2f4}.fce-reading b,.fce-reading span{display:block}.fce-reading span{margin-top:5px;font-size:12px;color:#667684}.fce-section-note{margin-top:-5px;color:#788795;font-size:12px}.fce-ranking{display:grid;grid-template-columns:1fr 1fr;gap:7px}.fce-ranking button{position:relative;overflow:hidden;display:flex;justify-content:space-between;gap:10px;padding:12px 13px;border:1px solid #dce3e8;border-radius:10px;background:#fff;color:#3e4e5d;text-align:left;cursor:pointer}.fce-ranking button span,.fce-ranking button strong{position:relative;z-index:1;font-size:12px}.fce-ranking button i{position:absolute;bottom:0;left:0;height:3px}.fce-ranking button:hover{border-color:#8fa7b5;transform:translateY(-1px)}@media(max-width:760px){.fce-toolbar{top:auto;bottom:12px;max-width:calc(100vw - 24px);flex-wrap:wrap;justify-content:center}.fce-toolbar-label{display:none}.fce-modal{padding:0}.fce-modal-card{width:100%;max-height:100%;height:100%;border-radius:0}.fce-modal-body{padding:24px}.fce-hero h2{font-size:27px}.fce-kpis{grid-template-columns:1fr 1fr}.fce-reading{grid-template-columns:1fr}.fce-ranking{grid-template-columns:1fr}}
    `;
