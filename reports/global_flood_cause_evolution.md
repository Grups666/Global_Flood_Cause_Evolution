...
- 研究首先在单个流域内判断大洪水生成条件是否发生持续变化，再检验这些局地信号是否在 **HydroBASINS 第5级水文分区**（HydroBASINS level 5，以下简称 **L5**）内形成更大尺度的一致模式。HydroBASINS 是一套沿河网拓扑划分、全球一致且逐级嵌套的子流域边界；L5 表示其中第5层空间单元，不是行政区划，也不是单个测站流域。
- 主样本包含 **59,048 场** POT/Q95 大洪水和 **2,624 个**长记录低雪流域；其中 **2,435 个流域**具有足够的所选事件年份，可估计至少一个主指标的单流域趋势。
- 五个连续主指标形成 **12,163 个流域—指标检验**（完整组合为 2,435 × 5 = 12,175；SSI 7日和30日因有效事件年份或时间跨度不足分别缺少 2 项和 10 项，缺失项不记为零）。共有 **378 个稳定候选**，但 **没有单流域信号通过5% Benjamini–Hochberg 假发现率控制（Benjamini–Hochberg false discovery rate，BH-FDR）**。因此目前的严格结论是：多数流域没有可由该观测网络稳定确认的长期生成条件变化；候选位置用于后续复核，而不是确证清单。
- 区域层在全部 1,475 个 L5—指标检验中有 **106 个**通过完整区域族 BH-FDR，**94 个**同时通过事件样本、SSI时间窗和留一检验。默认 **≥10%** 面积支持后，保留 **84 个强区域信号，分布于 36 个 L5**。
...
四个替代样本采用“一次只改变一个设定”的设计：POT/Q90 和 POT/Q97.5 只改变极端阈值，10日去簇 POT/Q95 只改变事件间隔规则，年最大洪水改变事件选择方法。因此，10日去簇并不是只有Q95才需要，而是以Q95主样本单独检验去簇敏感性；当前实验没有把阈值和去簇规则完全交叉。主样本中有 2,194 对相邻洪峰间隔小于10日，去簇样本将该数量降为 0；主样本事件风暴径流窗口重叠数为 0。
...
| 指标 | 可估计流域 | 未校正 p<0.05 | 稳定候选 | 跨流域BH-FDR | 候选负向 | 候选正向 |
|---|---:|---:|---:|---:|---:|---:|
...
共有 **378 个稳定候选**，同时包含增加和减少方向；**跨流域 BH-FDR 通过数为0**。因此本研究不支持“全球大多数流域都存在长期生成条件变化”，而支持“多数流域缺少严格长期证据，少数位置值得进一步核验”。

![单流域证据漏斗](assets/figure_03_strong_signal_rankings.png)

该图将未校正显著、稳定候选和 BH-FDR 证据分开，避免把稳健性和多重检验混为同一概念。

## 18. HydroBASINS 第5级水文分区（L5）是第二层空间问题

[HydroBASINS](https://www.hydrosheds.org/products/hydrobasins) 是由 HydroSHEDS 派生的全球子流域多边形数据集。它按照河流汇流关系和 Pfafstetter 编码，把全球陆地区域组织为 **1—12级逐层嵌套的水文单元**：层级越高，划分通常越细。本文使用标准版 HydroBASINS v1.c 的**第5级**，并将每一个第5级多边形简称为 **L5 水文分区**。这些边界描述排水系统的空间结构，不是国家或省级行政边界。

本研究的第一阶段分析单位仍然是**单个观测流域**：每个流域单独估计降雨集中度和前期湿润度趋势。L5 只在第二阶段提供统一的区域容器：先根据流域出口位置把若干观测流域归入同一 L5，再判断这些局地变化是否具有足够的面积支持和共同方向，因而可以解释为一个更大范围的水文模式。

**例子。** 一个700 km²的观测流域可能只是某个更大 L5 水文分区的一部分。它自己的趋势回答“这个流域是否发生变化”；同一 L5 内多个观测流域的汇总趋势回答“这种变化是否可能延伸到更大的相连水文区域”。因此，L5 结果不替代单流域结果，也不决定单流域结果是否保留。
