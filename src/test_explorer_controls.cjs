...
assert(!inspector.includes('Rainfall cutoff'));
explorer.setMechanism('All-Intensity');
explorer.showInspector(explorer.catchments.find(item => explorer.metric(item)));
assert(inspector.includes('Rainfall cutoff'));
assert.equal(explorer.data.meta.wetnessCalibration.dayOffset, -1);
assert.equal(explorer.data.meta.wetnessCalibration.classifiedPrimaryEvents + explorer.data.meta.wetnessCalibration.missingPrimaryEvents, explorer.data.meta.primaryEvents);
