...
Direct trends are estimable for at least one main metric in **2,435 catchments**. Across **12,163 catchment–metric tests**, **378 stable candidates** survive event-definition, declustering, annual-maximum, and leave-one-year checks, but none passes metric-wide 5% FDR. The strict direct result is therefore that most catchments do not show network-confirmed long-term shifts, while a smaller set remains useful for targeted follow-up.

HydroBASINS L5 is a second-stage regional lens. With the default **10% observed polygon-area support**, **84 strong regional signals in 36 L5 units** pass complete-family FDR plus event-sample, SSI-window, and leave-one-out checks. The web explorer allows 10%, 20%, 30%, 40%, and 50% area-support thresholds; this control changes regional interpretation without removing individual catchment results.
